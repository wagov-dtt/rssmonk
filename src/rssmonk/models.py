...
class FeedCreateRequest(BaseModel):
    ...
    visibility: Optional[ListVisibilityType] = Field(
        ListVisibilityType.PRIVATE, description="RSS feed visibility. Default to private"
    )
...
class FeedDeleteRequest(BaseModel):
    ...
    notify: Optional[bool] = Field(
        False,
        description="Set to true to send a generic campaign to everyone in the list to notify the end of the campaign",
    )
...
class CreateTemplateRequest(BaseModel):
    ...
    body_source: Optional[str] = Field(
        None, description="If type is campaign_visual, the JSON source for the email-builder tempalate"
    )
...
def subscribe_request_filter_value_check_common(value):
    ...
    """
    for _, freq_item in dict(
        value
    ).items():  # freq_item is FrequencyFilterType, don't need to check key (frequency) again
...
class SubscribeRequest(BaseModel):
    ...
    display_text: Optional[dict[Frequency, DisplayTextFilterType]] = Field(
        ..., description="The text for the filter above for email"
    )
...
class SubscribeAdminRequest(BaseModel):
    ...
    bypass_confirmation: Optional[bool] = Field(
        False, description="Bypass the temporary filter and email for confirmation."
    )
...
class SubscriptionPreferencesRequest(BaseModel):
    ...
    feed_url: Optional[HttpUrl] = Field(
        ...,
        description="RSS feed URL to subscribe to. If not supplied, attempt to obtain preferences of the account attached to the user",
    )
...
class UnsubscribeAdminRequest(BaseModel):
    ...
    bypass_confirmation: Optional[bool] = Field(
        False, description="Bypass any second email that might be send out for unsubscribing"
    )
...
class TemplateResponse(BaseModel):
    ...
    body: str = Field(..., description="HTML body of the template")
    body_source: Optional[str] = Field(
        ..., description="If type is campaign_visual, the JSON source for the email-builder tempalate"
    )
...
class ApiAccountResponse(BaseModel):
    ...
    api_password: str = Field(
        ..., description="Password that is generated for the API account and never revealed again"
    )
