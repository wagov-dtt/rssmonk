...
class TestRSSMonkUnsubscribe(ListmonkClientTestBase):
    ...
    def test_post_unsubscribe_non_admin_unsubscribe_request_failures(self):
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_2_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_3_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        assert "Value error, Token should not be empty" in response.text
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_4_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_5_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        assert "Value error, badly formed hexadecimal UUID string" in response.text
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_6_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
        assert "Value error, Token should not be empty" in response.text
        ...
        response = requests.post(
            RSSMONK_URL + "/api/feeds/unsubscribe", auth=HTTPBasicAuth(user, pwd), json=unsub_7_request
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_CONTENT, f"{response.status_code}: {response.text}"
    ...
    def test_post_unsubscribe_non_admin_unsubscribe_request_valid_data(self):
        ...
        response = self.admin_session.get(
            LISTMONK_URL + "/api/subscribers", params={"query": "subscribers.email='example@example.com'"}
        )
    ...
    def test_post_unsubscribe_admin_unsubscribe_admin_request(self):
        ...
        unsub_2_request = {
            ...
            "feed_url": "http://www.abc.net.au/rss",
    ...
    def test_post_unsubscribe_admin_unsubscribe_request_valid_data(self):
        ...
        response = make_admin_session().get(
            LISTMONK_URL + "/api/subscribers", params={"query": f"subscribers.email = '{email_address}'"}
        )
        assert response.json()["data"]["total"] == 0
